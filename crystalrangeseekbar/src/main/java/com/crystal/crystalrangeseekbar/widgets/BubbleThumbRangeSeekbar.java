...
public class BubbleThumbRangeSeekbar extends CrystalRangeSeekbar {
    ...
    private void drawBitmap(Canvas canvas, Drawable image, float left, float top, Paint paint) {
        image.setBounds((int) left, (int) top, (int) left + image.getIntrinsicWidth(), (int) top + image.getIntrinsicHeight());
        image.draw(canvas);
    }

    @Override
    protected void drawRightThumbWithImage(Canvas canvas, Paint paint, RectF rect, Drawable image) {
        if (isPressedRightThumb) {
